...
func (mm *metricsMiddleware) TrainGA(ctx context.Context, historyFilePath string) error {
	defer func(begin time.Time) {
		mm.counter.With("method", "train-ga").Add(1)
		mm.latency.With("method", "train-ga").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return mm.svc.TrainGA(ctx, historyFilePath)
}

func (mm *metricsMiddleware) TrainPSO(ctx context.Context, historyFilePath string) error {
	defer func(begin time.Time) {
		mm.counter.With("method", "train-pso").Add(1)
		mm.latency.With("method", "train-pso").Observe(time.Since(begin).Seconds())
	}(time.Now())

	return mm.svc.TrainPSO(ctx, historyFilePath)
}
